...
function Sidebar({ open, onClose, onToggle }) {
    ...
    const { getPendingDaak } = useDaak();
    const { isAdmin } = useAuth();

    const [daakOpen, setDaakOpen] = React.useState(false);
    const [adminOpen, setAdminOpen] = React.useState(false);

    const folderCounts = getFolderCounts();
    const pendingDaakCount = getPendingDaak().length;
    ...
    const menuItems = [
        ...
        { divider: true, label: 'Files' },
        {
            ...
            icon: <InboxIcon />,
            path: '/files/in-tray',
            badge: folderCounts['in-tray'],
        },
        {
            label: 'Draft',
            icon: <DraftsIcon />,
            path: '/files/draft',
            badge: folderCounts['draft'],
        },
        {
            label: 'Sent',
            icon: <SendIcon />,
        ...
        },
        {
            ...
            icon: <CabinetIcon />,
            path: '/files/cabinet',
            badge: folderCounts['cabinet'],
        },
        {
            label: 'Shared',
            icon: <ShareIcon />,
            path: '/files/shared',
            badge: folderCounts['shared'],
        },
        {
            label: 'Tracked',
            icon: <BookmarkIcon />,
            path: '/files/tracked',
            badge: folderCounts['tracked'],
        },
        {
            label: 'Archived',
            icon: <ArchiveIcon />,
                    ...
                    <Tooltip title={open ? '' : 'Daak'} placement="right">
                        ...
                        >
                            <ListItemIcon sx={{ minWidth: open ? 40 : 0, justifyContent: 'center' }}>
                                <Badge badgeContent={pendingDaakCount} color="warning" max={99}>
                                    <MailIcon />
                                </Badge>
                            </ListItemIcon>
                            {open && (
